...
void AksenMain(void)
 {
	char zeichen,
		 unerwartetes_zeichen = 0;							// um Eingabe von unerwarteten Zeichen abzufangen
	...
	serielle_init();										// Verbindung über serielle Schnittstelle initialisieren
	initialisiereServos();
	...
	{ 	
		...
		// erfolgreiche Übertragung, warte auf den nächsten Input
		// zurück zum Anfang
	}
}
...
unsigned char fuehreBefehleAus(struct servobefehl *befehlsliste, int anz_pakete){
	...
	//	for (k=0; k<2; k++)
		for (j=0; j<anz_pakete; j++)
		{
			...
			if (befehlsliste[j].servonummer == 2) 
			{
...
void initialisiereServos(void)
{
	...
	//int i;
	//for(i=0; i<2; i++){	
	//	servo_arc(i,90);								// alle Servos auf 90 Grad stellen
	//	sleep(1000);
	//	servo(i,1050);								// alle Servos auf Mittelstellung
	//}

}

void ueberpruefeChecksumme(void)
{
}
