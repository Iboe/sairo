...
public class RemoteControl extends javax.swing.JFrame {
	...
	private int lastPropellorSend = -1; // The last value send for this control,
										// used for new-input determination
	private int lastSailSend = -1; // The last value send for this control, used
									// for new-input determination
	private int lastRudderSend = -1; // The last value send for this control,
										// used for new-input determination
	...
	/**
	 * Sends a propellor adjustment command. Will ignore any value equalling the
	 * last value send to avoid unnecessary traffic.
	 * 
	 * @param value
	 *            to be send
	 */
	...
	/**
	 * Sends a sail adjustment command. Will ignore any value equalling the last
	 * value send to avoid unnecessary traffic.
	 * 
	 * @param value
	 *            to be send
	 */
	...
	/**
	 * Sends a rudder adjustment command. Will ignore any value equalling the
	 * last value send to avoid unnecessary traffic.
	 * 
	 * @param value
	 *            to be send
	...
	/**
	 * Converts a given value from -100 to +100 into the target range. Meant for
	 * converting the range of the slider values to a concrete value to be used
	 * as instruction value for the locomotion. Requires of the values to be
	 * following a general trend (min < nul < max).
	 * 
	 * @param value
	 *            to be converted
	 * @param min
	 *            minimum target value (equals -100 slider)
	 * @param nul
	 *            neutral target value (equals 0 slider)
	 * @param max
	 *            maximum target value (equals +100 slider)
	...
	private int convertToNativeValuePropellor(int value, int min, int nul, int max) {
		int myReturn;
		...
		if (value == 0) {
			...
			int nativeTick;
			if (value > 0) {
				myReturn = nul - (value * (Math.abs(max - nul)) / 100);
				// nativeTick = (max - nul);
			} else {
				myReturn = nul + (value * (Math.abs(nul - min)) / -100);
				// nativeTick = (nul - min);
			}
			// myReturn = ((value * nativeTick) / 100) + nul;
		}
	...
	private void initComponents() {
		...
		menuSettings_ResetMotor.setAccelerator(javax.swing.KeyStroke
				.getKeyStroke(java.awt.event.KeyEvent.VK_1,
						java.awt.event.InputEvent.CTRL_MASK));
		menuSettings_ResetMotor.setText("Motor nullstellen");
		menuSettings_ResetMotor
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		menuSettings_ResetSail.setAccelerator(javax.swing.KeyStroke
				.getKeyStroke(java.awt.event.KeyEvent.VK_2,
						java.awt.event.InputEvent.CTRL_MASK));
		menuSettings_ResetSail.setText("Segel nullstellen");
		menuSettings_ResetSail
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		menuSettings_ResetRudder.setAccelerator(javax.swing.KeyStroke
				.getKeyStroke(java.awt.event.KeyEvent.VK_3,
						java.awt.event.InputEvent.CTRL_MASK));
		menuSettings_ResetRudder.setText("Ruder nullstellen");
		menuSettings_ResetRudder
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		menuSettings_AutoSendCommands
				.setText("Steuerbefehle automatisch senden");
		menuSettings_AutoSendCommands
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
	...
	private void menuSettings_ResetMotorActionPerformed(
			java.awt.event.ActionEvent evt) {
		propellorSlider.setValue(0);
		propellorText.setText(convertToNativeValuePropellor(propellorSlider.getValue(),
				PROPELLOR_MIN, PROPELLOR_NULL, PROPELLOR_MAX) + "");
	}

	private void menuSettings_ResetSailActionPerformed(
			java.awt.event.ActionEvent evt) {
		sailSlider.setValue(0);
		sailText.setText(convertToNativeValue(sailSlider.getValue(), SAIL_MIN,
				SAIL_NULL, SAIL_MAX) + "");
	...
	private void buttonSendCommandsActionPerformed(
			java.awt.event.ActionEvent evt) {
		sendCommands();
	}
	...
	public static void main(String args[]) {
		...
		try {
			...
			java.util.logging.Logger.getLogger(RemoteControl.class.getName())
