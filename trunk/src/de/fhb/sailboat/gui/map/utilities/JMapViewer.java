...
public class JMapViewer extends JPanel implements TileLoaderListener {
	...
	public void setDisplayToFitMapRectangle() {
		...
		for (MapRectangle rectangle : mapRectangleList) {
			...
			y_max = Math.max(y_max, OsmMercator.LatToY(rectangle.getTopLeft()
					.getLat(), mapZoomMax));
			x_min = Math.min(x_min, OsmMercator.LonToX(rectangle.getTopLeft()
					.getLon(), mapZoomMax));
		...
		if (mapRectanglesVisible && mapRectangleList != null) {
			for (MapRectangle rectangle : mapRectangleList) {
				...
				if (topLeft != null && bottomRight != null) {
					Point pTopLeft = getMapPosition(topLeft.getLat(),
							topLeft.getLon(), false);
					Point pBottomRight = getMapPosition(bottomRight.getLat(),
							bottomRight.getLon(), false);
	...
	private void paintAttribution(Graphics g) {
		...
		Rectangle2D termsStringBounds = g.getFontMetrics().getStringBounds(
				"Background Terms of Use", g);
		...
		String attributionText = tileSource.getAttributionText(zoom, topLeft,
				bottomRight);
