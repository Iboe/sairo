...
/**
 * Serial Port Factory
 * parallel Version for OutputStream
 *  ==> needs to be merged with original sairoComm!
 * @author schmidst
 *
 */
public class sairoComm2 {
  ...
  private boolean eventreader = false;	// Use Events instead of Constant-Stream
  private boolean charReader = true;	// Read Bitwise/Charwise instead of Linewise (\n or \r seperated)
  ...
  public sairoComm2(String comPort, int baudrate, boolean eventreader, boolean charRead) {
	    this.comPort = comPort;					// ComPort (Win. e.g.: Com3, Unix e.g.: /dev/ttyS0)
  ...
  /**
   * The Connect method, that starts the writing of the dataString and does all the
   * connecting stuff
   */
  public void connect() throws Exception {
	  ...
	  if(this.status == 0) {
		  ...
		  // Get an OutputStream by calling the sairoConn.getOutputStream method


			  this.outputStream = this.serialPort.getOutputStream();
		  
		  // Get an InputStream by calling the sairoConn.getInputStream method

			  this.inputStream = this.serialPort.getInputStream();
		  
			  // Start the Thread with the OutputStream and an Instance of the active object type sairoConn
			  //Thread output = (new Thread(new SerialWriter(this.outputStream, this)));
			  //output.start();
	
			  // TODO EventListener
			  if (this.eventreader) {
				  this.serialPort.addEventListener(new SerialEventReader(this.inputStream, this));
		          this.serialPort.notifyOnDataAvailable(true);
		          
			  } else {
				// Start the Thread with the InputStream and an Instance of the active object type sairoConn
		          Thread input = (new Thread(new SerialReader(this.inputStream, this)));
		          input.start();  
			  }
	        ...
	        while (keepRunning) {
    	      ...
    	      try {
    	        while ((len = this.in.read(buffer)) > -1) {
    	          ...
    	          sairoInstance.setStrInput(new String(buffer, 0, len));
    	          }
    	        }
    	      } catch (IOException e) {
    	        e.printStackTrace();
