...
public class Geocoordinate implements Comparable<Geocoordinate>
{
	...
	{
		if(mode=='D')
			return (latdeg+latmin/60)+"°"+' '+(londeg+lonmin/60)+'°';
		else if(mode=='M')
			return latdeg+"°"+Math.abs(latmin)+'\''+' '+londeg+'°'+Math.abs(lonmin)+'\'';
		else
			return latdeg+"°"+Math.abs(latmin)+'\''+Math.abs(latsec)+'\"'+' '+londeg+'°'+Math.abs(lonmin)+'\''+Math.abs(lonsec)+'\"';
	}
}